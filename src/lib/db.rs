...
pub mod tests {
    ...
    fn test_serialize_yubikey_entry_type() {
        expect!(serde_json::to_string(&YubikeyEntryType::ChallengeResponse)).to(be_ok().value(r#""ChallengeResponse""#.to_string()));
        expect!(serde_json::to_string(&YubikeyEntryType::HybridChallengeResponse)).to(be_ok().value(r#""HybridChallengeResponse""#.to_string()));
    }

    #[test]
    fn test_serialize_volume_id() {
        expect!(serde_json::to_string(&VolumeId::new(None, Uuid::nil()))).to(be_ok().value(r#"{"name":null,"id":{"uuid":"00000000-0000-0000-0000-000000000000"}}"#.to_string()));
        expect!(serde_json::to_string(&VolumeId::new(Some("foobar".to_string()), Uuid::nil()))).to(be_ok().value(r#"{"name":"foobar","id":{"uuid":"00000000-0000-0000-0000-000000000000"}}"#.to_string()));
    }
    ...
    fn test_serialize_passphrase_entry() {
        ...
        expect!(serde_json::to_string(&entry)).to(be_ok().value(r#"{"PassphraseEntry":{"volume_id":{"name":null,"id":{"uuid":"00000000-0000-0000-0000-000000000000"}}}}"#.to_string()));
    }
