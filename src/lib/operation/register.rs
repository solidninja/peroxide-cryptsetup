use context;
use db;
use operation::{OperationError, PerformCryptOperation, RegisterOperation, Result, UserDiskLookup};
...
impl<Context: context::WriterContext> RegisterOperation<Context> {
    ...
    fn to_entry<P>(&self, path: &P) -> Result<db::DbEntry>
    where
        P: AsRef<Path>,
    {
        ...
        match self.entry_type {
            ...
            db::DbEntryType::Passphrase => Ok(db::DbEntry::PassphraseEntry { volume_id }),
            other => Err(OperationError::BugExplanation(format!(
