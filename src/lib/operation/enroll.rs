...
use operation::{PerformCryptOperation, EnrollOperation, OperationError, UserDiskLookup, Result, PasswordPromptString, LuksDevice, NewContainerParameters};
use model::{DbEntryType, DbEntry, VolumeId};
use io::{FileExtensions, KeyWrapper};

use context;
use context::{WriterContext, ReaderContext, InputContext};

impl<Context, BackupContext> PerformCryptOperation for EnrollOperation<Context, BackupContext>
    where Context: WriterContext + InputContext,
          ...
          BackupContext: ReaderContext + InputContext
{
    ...
    fn create_or_format_devices<P: AsRef<Path>>(&self, device_paths: &[P]) -> Result<Vec<cryptsetup_rs::Luks1CryptDeviceHandle>> {
        device_paths
            .iter()
            .map(|path| {
                match self.new_container {
    ...
    fn format_container<P: AsRef<Path>>(&self, params: &NewContainerParameters, path: P) -> Result<cryptsetup_rs::Luks1CryptDeviceHandle> {
        ...
        let device = cryptsetup_rs::format(path.as_ref())
            .and_then(|format| format.luks1(cipher_split[0],
                                            cipher_split[1],
                                            &params.hash,
                                            params.key_bits,
                                            None))
    ...
    fn get_preexisting_entry(&self, device: &LuksDevice) -> Option<DbEntry> {
        if self.new_container.is_some() {
            ...
            self.backup_context
                ...
                .or_else(|| Some(DbEntry::PassphraseEntry { volume_id: VolumeId::new(None, device.uuid()) }))
        }
    }

    fn enroll_devices_with_entries(&self, devices_with_entries: Vec<(cryptsetup_rs::Luks1CryptDeviceHandle, Option<DbEntry>)>) -> Result<()> {
        ...
        let enroll_device = |cd: &mut LuksDevice, entry: Option<&DbEntry>| {
            ...
            // enroll
            cd.add_keyslot(new_key.as_slice(),
